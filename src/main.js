...
function setLeftVisibility(visible) {
  ...
  if (!left || !btn) return;
  if (visible) {
    left.style.opacity='1'; left.style.transform = "translateX(0px)"; right.style.transform = "translateX(0px)"; btn.innerHTML = '&#10005;'; btn.title = 'Hide menu';
  } else {
...
const dataPst = document.getElementById('data-pst');
if (dataPst) {
  dataPst.addEventListener('click', async () => {
    try {
      ...
      alert('Failed to read or parse clipboard data.');
    }
  });
}
